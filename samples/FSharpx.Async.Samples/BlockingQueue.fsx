...
// timeout is larger, consumer will be blocked. Otherwise, producer
// will be blocked.
...
async { 
  for i in 0 .. 10 do 
    ...
    printfn "Added %d (Thread# %A)" i Thread.CurrentThread.ManagedThreadId }
|> Async.Start
...
async { 
  while true do
    ...
    printfn "Got %d (Thread# %A)" v Thread.CurrentThread.ManagedThreadId}
|> Async.Start