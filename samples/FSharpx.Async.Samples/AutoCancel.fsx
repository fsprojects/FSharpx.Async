...
let op = async {
    ...
    try 
      while true do
        ...
        | (n, reply:AsyncReplyChannel<unit>) ->
            ...
            printfn "%d (Thread# %A)" n Thread.CurrentThread.ManagedThreadId
            reply.Reply(())        
    finally 
  ...
  do! Async.Sleep(100)
  printfn "workflow completed (Thread# %A)" Thread.CurrentThread.ManagedThreadId}

printfn "starting workflow (Thread# %A)" Thread.CurrentThread.ManagedThreadId
Async.Start(op)